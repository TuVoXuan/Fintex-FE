...
export default function Chat() {
    ...
    const [activedConversation, setActivedConversation] = useState<string>(conversationId);
    ...
    useEffect(() => {
        if (sConversations.length === 0) {
            dispatch(getConversations()).then((data) => {
                ...
                const activeConvIndex = convs.findIndex((item) => item._id === activedConversation);
                if (activeConvIndex >= 0) {
                    setActivedConversation(convs[activeConvIndex]._id);
                } else {
                    setActivedConversation(convs[0]._id);
                }
                if (mqtt) {
                    if (activeConvIndex >= 0) {
                        mqtt.setConversation(convs[activeConvIndex]._id);
                    } else {
            ...
            const activeConv = sConversations.find((item) => item._id === activedConversation);
            if (activeConv) {
                setActivedConversation(activeConv._id);
            } else {
                setActivedConversation(sConversations[0]._id);
            }
            if (mqtt) {
                if (activeConv) {
                    mqtt.setConversation(activeConv._id);
                } else {
